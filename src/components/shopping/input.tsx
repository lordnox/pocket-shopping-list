...
export const ShoppingInput: Component<{ onEnter: (data: ShoppingItemCreate) => void }> = (props) => {
  ...
  const enterItem: JSX.EventHandlerUnion<HTMLButtonElement, MouseEvent> = async (event) => {
    ...
    props.onEnter({ name, price })
  }

  return (
    <form class="flex gap-2 m-2">
      <div class="">
        <Input for="name" label="Name" placeholder="Name" ref={nameInputElement!} required />
      </div>
      <div class="">
        <Input for="price" label="Preis" placeholder="Preis" ref={priceInputElement!} required type="number" />
      </div>
      <button
